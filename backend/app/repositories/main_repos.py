...
class DishRepo:
    ...
    async def get_dish_by_id(self, dish_id) -> DishModel :
        stmt = select(DishModel).where(DishModel.id == dish_id)
        result = await self.db.scalars(stmt)
        return result.first()


    async def create_dish(self, dish: DishCreate) -> DishModel:
    ...
    async def update_dish(self, dish_id: int, new_dish: DishUpdate | DishCreate):
...
class CategoryRepo:
    ...
    async def get_all_categories(self) -> Sequence[CategoryModel]:
        result = await self.db.scalars(select(CategoryModel))
        return result.all()


    async def get_category_by_id(self, category_id) -> CategoryModel:
        stmt = select(CategoryModel).where(CategoryModel.id == category_id)
        result = await self.db.scalar(stmt)
        return result

    async def create_category(self, category: CategoryCreate) -> CategoryModel:
    ...
    async def update_category(self, category_id: int, new_category: CategoryUpdate) -> CategoryModel:
