...
from app.schemas.cart import CartItemCreate, CartItemRead, CartWithItemsResponse
from app.services.cart import CartService

router = APIRouter(prefix="/cart", tags=["Корзина"])


@router.get("/", response_model=CartWithItemsResponse, summary="Просмотр корзины")
async def get_cart(
...
@router.post("/items", status_code=201, response_model=CartItemRead, summary="Добавить блюдо")
...
@router.patch("/items", status_code=204, summary="Обновить количество")
...
@router.delete("/items/{dish_id}", status_code=204, summary="Удалить из корзины")
