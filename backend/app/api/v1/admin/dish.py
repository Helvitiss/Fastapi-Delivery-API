...
from app.schemas.dish import DishCreate, DishRead, DishUpdate
...
@router.get("/{dish_id}", response_model=DishRead, summary="Детали")
...
@router.post("/", response_model=DishRead, status_code=201, summary="Создать")
...
@router.post("/{dish_id}/photo", summary="Загрузить фото")
...
@router.patch("/{dish_id}", response_model=DishRead, summary="Обновить")
async def update_dish(
    dish_id: int, dish: DishUpdate, menu_service: DishService = Depends(get_dish_service)
):
    return await menu_service.update_dish(dish_id, dish)


@router.delete("/{dish_id}", status_code=204, summary="Удалить")
