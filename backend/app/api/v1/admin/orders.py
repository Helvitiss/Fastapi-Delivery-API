...
@router.get("/", response_model=list[OrderRead], summary="Все заказы")
async def list_all_orders(order_service: OrderService = Depends(get_order_service)):
...
@router.patch("/{order_id}/status", response_model=OrderRead, summary="Изменить статус")
