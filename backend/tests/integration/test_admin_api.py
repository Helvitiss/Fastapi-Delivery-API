...
from app.models import CategoryModel, OrderModel, UserModel
...
async def test_admin_create_category(client, admin_user, mock_category_service):
    ...
    response = await client.post("/admin/categories/", json={"name": "Admin Cat"})
...
async def test_admin_update_order_status(client, admin_user, mock_order_service):
    ...
    response = await client.patch("/admin/orders/1/status", json={"status": "completed"})
...
async def test_admin_upload_dish_photo(client, admin_user, mock_dish_service):
    ...
    response = await client.post("/admin/dishes/1/photo", files=files)
